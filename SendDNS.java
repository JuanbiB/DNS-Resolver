...
class SendDNS {
		...
		if (recursive != 1){
			...
			for (DNS.ResourceRecord rec: result.rrlist) {
				if (rec.type == 5 && rec.value != null) {
					...
					System.out.println("Hostname: " + hostname + " resolved to: " + rec.value);
					if (CNAMES.size() > 0)
						System.out.println("Alises: " + CNAMES);
					return;
				}
			}
		}
			...
			if (result != null) {
				...
				for (DNS.ResourceRecord rec: result.rrlist) {
					...
					// For reverse queries our answer is going to come in the way of a PTR package.
						...
						System.out.println("Host could not be resolved: " + hostname);
						System.exit(0);
					}
				}
				if (found) break;
	...
	static DNS sendRequest(byte[] serveraddr, String hostname) throws IOException {
		...
		int querylen = DNS.constructQuery(query, 1500, hostname, reverse, recurse);
